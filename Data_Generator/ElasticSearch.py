...
try:
    for i in range(kolvo_materials):
        docs.append({
            'description': 'Версионирование и поддержка целостности микросервисной системы. Устранение ада зависимостей на примере решения Банка. Коммуникация сервисов между собой. Почему важны стандарты и какие они бывают. CDC (client driven contracts) и кононические модели данных. Under- и Overfetching данных. GraphQL как современный язык разработки контрактов микросервисов. Многоязычные среды. Альтернативы и проблемы. Событийная модель и брокеры сообщений. Децентрализованный подход к сервисам аутентификации и авторизации. Ролевая модель и уровни доступа. Доступ клиент-сервер и сервер-сервер. Токены, API key, использование различных каналов аутентификации.',
